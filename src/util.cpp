...
const char * const BITCOIN_CONF_FILENAME = "predatorcoin.conf";
const char * const BITCOIN_PID_FILENAME = "predatorcoind.pid";
 ...
 * by well-meaning predatorcoin adding mutexes in the most straightforward way.
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "predatorcoin" is a composite category enabling all PRDCoin-related debug output
            if(ptrCategory->count(string("predatorcoin"))) {
...
#else
    const char* pszModule = "predatorcoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PRDCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PRDCoin
    // Mac: ~/Library/Application Support/PRDCoin
    // Unix: ~/.predatorcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PRDCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PRDCoin";
#else
    // Unix
    return pathRet / ".predatorcoin";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty predatorcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override predatorcoin.conf
