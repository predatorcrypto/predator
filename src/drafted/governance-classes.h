 ...
 *
 *   // network
 *   CPRDCoinNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.predatorcoin.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CPRDCoinNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CPRDCoinNetwork(UniValue objIn)
    {
...
// // can be under: PRDCoinNetwork
// //   -- signature requirements : Key1(User)
// class CPRDCoinNetworkVariable : public CGovernanceObject
// {
// private:
