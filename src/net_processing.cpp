...
#if defined(NDEBUG)
# error "Predator coin cannot be compiled without assertions."
#endif
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /* 
        PRDCoin Related Inventory Messages
        ...
        if (!AlreadyHave(inv) && AcceptToMemoryPool(mempool, state, tx, true, &fMissingInputs))
        {
            ...
            for (unsigned int i = 0; i < vWorkQueue.size(); i++)
            {
                     ...
                     ++mi)
                {
                    ...
                    // resolution (that is, feeding predatorcoin an invalid transaction based on LegitTxX in order to get
